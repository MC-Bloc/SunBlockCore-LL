...
**`CPUPowerDraw` is read directly from Intel RAPL powercap counters** (`/sys/devices/virtual/powercap/*/energy_uj`) by `hardware._read_cpu_power_draw()` — there is no external script and no `POWER_DRAW_SCRIPT_ADDR` configuration anymore (see Decision 10 in §10, which replaced the script-based approach this section used to describe). Power is computed as the energy delta between successive `parse_data()` calls divided by the actual elapsed wall-clock time, so it stays accurate regardless of `READ_INTERVAL`. The underlying `energy_uj` files are root-only on most distros; a direct read is tried first, falling back to one batched `sudo -n cat` call if needed (see `docs/DEPLOYMENT.md` §6 for the sudoers entry). If RAPL isn't present at all, or isn't readable even via the sudo fallback, `CPUPowerDraw` is simply `0.0` (a `float`, matching the `REAL` column in `solardata`) — the polling loop is never put at risk by this value, by design (see Decision 9 in §10 for the original guard that made this safe).
...
**Superseded by Decision 10 below** — there is no longer a `POWER_DRAW_SCRIPT_ADDR`/external script at all, but the underlying lesson (never let `CPUPowerDraw` be capable of taking down `polling_loop`) carried forward directly into the new implementation.

### Decision 10: Replace `POWER_DRAW_SCRIPT_ADDR` external script with direct RAPL integration

**Chosen**: `hardware.py` reads Intel RAPL powercap counters (`/sys/devices/virtual/powercap/*/energy_uj`) directly in Python — scanning once per process lifetime (`_scan_power_caps()`), then on each `parse_data()` call computing power as the energy delta since the previous call divided by actual elapsed time (`_read_cpu_power_draw()`). Domain filtering (skip `intel-rapl-mmio` duplicates, skip `core`/`uncore`/`psys` sub-domains already summed into `package`) mirrors the old external `power_draw.sh` script's logic exactly. The root-only `energy_uj` files are read directly first; if that raises `PermissionError`, a single batched `sudo -n cat <path...>` call is the fallback. **The sudoers entry for that fallback must list exact literal paths, never a wildcard** — `scripts/deploy.sh` discovers them automatically by importing `hardware.py` and calling `_scan_power_caps()` (see `docs/DEPLOYMENT.md` §6 for why, and what goes wrong with a wildcarded version).  
**Rejected**: keeping the external shell script and `POWER_DRAW_SCRIPT_ADDR` config — that script had its own internal privilege-elevation logic (sudo/pkexec probing) and a fixed ~1-second polling loop with its own `sleep`, neither of which is needed when the same logic runs inside `parse_data()` (which already has an accurate elapsed-time measurement available, and is itself called on a controlled cadence by `polling_loop`).  
**Rationale**: removes an entire external dependency (a script that had to be separately installed, made executable, and granted its own sudoers entry on every deployment) in favor of code that lives in the repo, is covered by the same review/testing as everything else, and computes power more accurately (real elapsed time instead of an assumed 1-second gap). It also closes a class of bug the script-based approach was vulnerable to: a negative energy delta (counter wraparound/reset between reads) is explicitly guarded against and skips that tick's reading rather than reporting a nonsensical value — the original script had no equivalent protection. `CPUPowerDraw` still defaults to `0.0` whenever RAPL is absent or unreadable, preserving the safety property established in Decision 9 (this value must never be capable of stopping `polling_loop`).
