...
- **`CPUPowerDraw` is read directly from Intel RAPL powercap counters and must stay guarded.** There is no external script and no `POWER_DRAW_SCRIPT_ADDR` anymore — `hardware._read_cpu_power_draw()` scans `/sys/devices/virtual/powercap/*/energy_uj` once (`_scan_power_caps()`, cached in the module-level `_power_cap_paths`) and computes power as the energy delta since the previous call divided by actual elapsed time. It returns `0.0` (never raises) whenever RAPL is absent, the counter files aren't readable (even after the `sudo -n cat` fallback), or a counter wraparound produces a negative delta. **This guard is load-bearing for the same reason as before**: an earlier version of this feature called an unguarded external script via `subprocess.run([config.POWER_DRAW_SCRIPT], ...)`, which threw `TypeError` and silently killed the entire polling loop when the script path was unset — symptom: "the server runs but isn't broadcasting any events on the socket," with no other error visible (see `docs/DEVELOPMENT_HISTORY.md` Session 12). Whatever `CPUPowerDraw`'s implementation, it must never be capable of raising out of `parse_data()`.
